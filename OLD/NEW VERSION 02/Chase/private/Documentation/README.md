...
An experiment made with PyGame.
...
__`duration`__       | vector of number of seconds that each trial lasts. For example, 30,10, ... __the integers should be separated by comma__.
__`chaseRate`__       | vector of chase probability per `turnRate`. __the integer should be between 0 to 100 and separated by comma__.  
__`chaseAngle`__      | vector of chase angle for each trial. the integer can be between `1` ro `180`. __the integers should be separated by comma__. (`1` is practically straightline chase, given the size of the game objects and screen)
...
![](..\resources\image\available colors.jpg)
